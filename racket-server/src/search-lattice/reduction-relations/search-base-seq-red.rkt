...
(define delay-local
  (reduction-relation
   search-base-seq-lang
   #:domain f
   [--> (in-hole KBranch (in-hole KBase ((suspend g tag) σ)))
        (in-hole KBranch (in-hole KBase (delay (g σ))))
        "delay/suspend-goal"]
   [--> (in-hole KBranch (in-hole KBase ((delay f_1) × g c)))
        (in-hole KBranch (in-hole KBase (delay (f_1 × g c))))
...
(define scoped-conj-local
  (reduction-relation
   search-base-seq-lang
   #:domain f
   [--> (in-hole KBranch (in-hole KBase ((Freshened c_1 f_left) × g c_2)))
        (in-hole KBranch (in-hole KBase (Freshened c_1 (f_left × g c_2))))
...
(define search-extra
  (reduction-relation
   ...
   [--> (in-hole KBranch (in-hole KBase (((⊤ σ_new) + f_rest) × g c)))
        (in-hole KBranch (in-hole KBase ((g σ_new) <-+ (f_rest × g c))))
        "search-base-seq/continue-left-prefix-answer"]
   [--> (in-hole KBranch (in-hole KBase ((Bounced + f_rest) × g c)))
        (in-hole KBranch (in-hole KBase (Bounced + (f_rest × g c))))
