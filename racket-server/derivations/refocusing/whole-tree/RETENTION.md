...
| [`reference/lean/`](reference/lean/README.md) | Independent lean reference.  Its source grammar, WF, Ktoy/Kmk kernels, 20 control rules, exact 23/27 source inventories, grammatical decomposition, contracts, and independent direct `D` steps are executable.  Retain through the later lean `Z/M/B/Big`, reference-Q, and modular-family validation; the present decomposition checkpoint is not a complete derivation. |
| [`q/reference/`](q/reference/README.md) | Marked-to-lean source and decomposition correspondence.  Retain its explicit source constructor map, five ranked stutters, alpha-aware allocation law, rule-complete bounded source witnesses, and reconstruct/project/re-decompose `Q_D` square.  Extend it stage by stage only after each independent lean stage exists; do not use it to manufacture either reference. |
