...
(require rackunit
         ...
         (only-in "../strict-search/shared/wf.rkt" wf-s-rel?)
         (only-in "../strict-search/retained-scope/relations.rkt" goal-body)
         "../strict-search/shared/kernel-equations.rkt"
...
(module+ test
  ...
  (test-case "prescribed native squares for finite no-allocation fragment"
    ...
    (define goals
      (append seeds (map delay seeds)
              (list '((sym "A") != (sym "B") (label "disequality-success"))
                    '((sym "A") != (sym "A") (label "disequality-fail"))
                    '((sym "A") =? (sym "B") (label "unify-fail")))
              (for*/list ([left (in-list (append seeds (map delay seeds)))]
                          [right (in-list (append seeds (map delay seeds)))])
                (or-goal left right))
              (for*/list ([left (in-list seeds)] [right (in-list seeds)])
                (and-goal (or-goal (delay left) right) (delay (atom "then"))))
              (for*/list ([a (in-list seeds)] [b (in-list seeds)] [c (in-list seeds)])
                (or-goal (delay a) (or-goal (delay b) (delay c))))))
