...
Here "lattice" means the earlier dormant-branch semantics under `src/search-lattice/`.
...
The existing [retained-scope Strict interpreter](../strict-search/retained-scope/interpreter.rkt)
...
```sh
raco test racket-server/derivations/scheduler-family/barriers-tests.rkt
```
...
The [dormant-branch interpreter](interpreter.rkt) changes demand as well as
scheduling. It is compared through its independently stepped
[source presentation](source.rkt), separately from the unchanged Strict
