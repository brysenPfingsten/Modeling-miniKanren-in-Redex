# Earlier dormant-branch semantics: interpreter derivations

This directory derives interpreters and machines for the **earlier
dormant-branch semantics** under `src/search-lattice/`. The current GUI uses
[strict S matrix scheduler rows](../strict-search/matrix/scheduler-source.rkt):
...
References below to the native lattice describe the earlier sources;
...
This directory is an investigation beside
[strict-search](../strict-search/README.md) and
[distributed-search](../distributed-search/README.md). It is not another
allocation matrix row or a replacement GUI backend.

## The common interpreter and its variations

[interpreter.rkt](interpreter.rkt) starts from the equations in
[retained-scope/interpreter.rkt](../strict-search/retained-scope/interpreter.rkt).
...
merge is rejected for DFS and Flip. The grammars in [source.rkt](source.rkt)
...
example with `racket racket-server/derivations/scheduler-family/show.rkt`.
...
|---|---|
| [interpreter.rkt](interpreter.rkt) | Direct equations, real computation closures, and separate public consumer |
| [cps.rkt](cps.rkt) | Explicit continuations for demand, bind, commitment and resumption |
| [data.rkt](data.rkt), [defunc.rkt](defunc.rkt) | Actual closure/continuation families as data; functional kernel Outcomes become native data Outcomes here |
| [derive.rkt](derive.rkt), [machine.rkt](machine.rkt) | Shared checked tail-position transformation generates 13 PC clauses and a data dispatch loop; no hand-copied lattice transitions |
| [source.rkt](source.rkt) | Independently stated grammars, contexts, named contractions, decomposition, plugging, retained scope, public advance and source orientation erasure |
| [functional-source.rkt](functional-source.rkt) | Structural machine readback, policy/Γ/scope validation, exact prescribed spans and administrative rank |
| [lattice-map.rkt](lattice-map.rkt) | Structural source/native map, label refinement, public span and administrative weight; explicit domain restriction |
| [orientation.rkt](orientation.rkt) | Independent full native Railroad→Flip map and exhaustive native label map |
| [BARRIERS.md](BARRIERS.md) | Strictness/divergence counterexamples, allocation and provenance discrepancies |
| [all.rkt](all.rkt) | One gate for the investigation, with no runtime import from the GUI |
...
The combined headless gate passed **3,693 tests** on 2026-09-07, including
this investigation's **191 cases** and all **205 application HEADLESS cases**
with zero failures or errors. Those counts are included in the total, not
additive. The generated-machine freshness check also passed.
...
[Compiler-driven barriers](barriers-tests.rkt) hold compilation fixed, and the
