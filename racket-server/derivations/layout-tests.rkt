...
(module+ test
  (test-case "the current derivation account has no experimental or application dependencies"
    (define (current? path)
      (and (inside? path directory) (not (inside? path experiments))))
    ;; Include the aggregate and test submodules, not just runtime providers.
    (define current-files (filter current? (source-files directory)))
    (check-not-false (member (simplify-path (build-path directory "all.rkt")) current-files))
    (check-closure current-files current?))

  (test-case "the application uses only live source and packaged matrix/shared providers"
    (define application-files (source-files application))
    (check-true (pair? application-files))
    (check-closure application-files
                   (lambda (path)
                     (and (or (inside? path application)
                              (inside? path matrix) (inside? path shared))
