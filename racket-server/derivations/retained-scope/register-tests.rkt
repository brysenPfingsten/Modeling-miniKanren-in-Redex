...
(module+ test
  ...
  (test-case "independent register banks can interleave across public Delay boundaries"
    (define left (r:initial paused))
    (define right (r:initial yes))
    (define (interleave)
      (define right-before (r:decode right))
      (define right-steps (r:Registers-steps right))
      (define left-live? (r:step! left))
      (check-equal? (r:decode right) right-before)
      (check-equal? (r:Registers-steps right) right-steps)
      (define left-after (r:decode left))
      (define left-steps (r:Registers-steps left))
      (define right-live? (r:step! right))
      (check-equal? (r:decode left) left-after)
      (check-equal? (r:Registers-steps left) left-steps)
      (when (or left-live? right-live?) (interleave)))
    (interleave)
    (check-equal? (r:decode left) (m:Halted (m:run paused)))
    (check-equal? (r:decode right) (m:Halted (m:run yes)))
    (define pending (r:Registers-r0 left))
    (set! left (r:from-machine (m:Call 'advance/d (list pending '() (KDone)))))
    (set! right (r:initial paused))
    (interleave)
    (check-equal? (r:decode left) (m:Halted (m:resume-once pending)))
    (check-equal? (r:decode right) (m:Halted (m:run paused)))
    (for ([bank (in-list (list left right))])
      (check-equal? (cdr (r:register-values bank)) '(#f #f #f #f))))
